                ...
                else
                {
                    ...
                    if (drawMode != DrawMode.Tower && (mouse.LeftButton == ButtonState.Pressed || mouse.RightButton == ButtonState.Pressed) && hoveredCoord.X >= 0 && hoveredCoord.X < ParentMap.Layout.GetLength(1) && hoveredCoord.Y >= 0 && hoveredCoord.Y < ParentMap.Layout.GetLength(0))
                    {
                        ...
                        if (mouse.LeftButton == ButtonState.Pressed)
                        {
                            ...
                            {
                                case DrawMode.Void:
                                    ParentMap.Layout[hoveredCoord.Y, hoveredCoord.X] = ' ';
                                    break;
                                case DrawMode.WallPath:
                                    ParentMap.Layout[hoveredCoord.Y, hoveredCoord.X] = '0';
                                    break;
                                case DrawMode.Point:
                                    if (MapEditorSpawnPoints.Count < 9) //single digits only (1–9)
                                    {
                        ...
                        else if (mouse.RightButton == ButtonState.Pressed)
                        {
                            switch (drawMode)
                            {
                                ...
                                case DrawMode.WallPath:
                                    ParentMap.Layout[hoveredCoord.Y, hoveredCoord.X] = hoveredCoord.X % 2 == 0 ? '\'' : '.';
                                    break;
                                case DrawMode.Point:
                                    if (MapEditorGoalPoints.Count < 9)
                                    {
            ...
            if (CurrentGame.gameState != GameState.MapEditor)
            {
                ...
                if (newTileRingActive || towerTileRingActive || priorityRingActive)
                {
                    ...
                    else if (towerTileRingActive)
                    {
                        ...
                        {
                            case 0:
                                TileRingInfoBox = new TowerInfoBox(selectedTower, belowTilePos, false);
